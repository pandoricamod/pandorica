...
public class PandoricaCommon {
    public static class StaticMeleeAttackGoal {
            ...
            if (l - lastUpdateTime < 20L) {
                ...
                if (target == null) {
                    ...
                    if (path != null) {
                        ...
                        return squaredMaxAttackDistance >= mob.squaredDistanceTo(target.getX(), target.getY(),
                                target.getZ());
                    }
                }
            }
        ...
        public static void tick(MobEntity mob, int updateCountdownTicks, boolean pauseWhenMobIdle, double targetX,
                double targetY, double targetZ, double speed, int ticksUntilAttack) {
            LivingEntity target = mob.getTarget();
            ...
            if ((pauseWhenMobIdle || mob.getVisibilityCache().canSee(target)) && updateCountdownTicks <= 0
                    && (targetX == 0.0D && targetY == 0.0D && targetZ == 0.0D
                            || target.squaredDistanceTo(targetX, targetY, targetZ) >= 1.0D
