...
public class SoulBrickWallBlock extends WallBlock {
    public static final BooleanProperty LAVALOGGED = BooleanProperty.of("lavalogged");

    public SoulBrickWallBlock() {
        super(AbstractBlock.Settings.copy(PandoricaBlocks.BlockWithDecorInfo.SOUL_BRICKS.base).lightLevel(blockStatex -> blockStatex.get(LAVALOGGED) ? 6 : 0));
        this.setDefaultState(this.getDefaultState().with(LAVALOGGED, false));
    }

    @Override
    protected void appendProperties(StateManager.Builder<Block, BlockState> stateManager) {
        stateManager.add(UP, NORTH_SHAPE, EAST_SHAPE, WEST_SHAPE, SOUTH_SHAPE, WATERLOGGED, LAVALOGGED);
    }
    ...
    public void onBroken(IWorld world, BlockPos pos, BlockState state) {
        if (state.get(LAVALOGGED)) world.setBlockState(pos, Blocks.LAVA.getDefaultState().with(FluidBlock.LEVEL, 3), 11);
    }
}
